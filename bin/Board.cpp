...
Board::Board(int size) {
    ...
    grid = new VArray< VArray<Space*>* >(0, size);
    overlay = new VArray< VArray<char>* >(0, size);

    // initialise tiles
    for(int i=0; i<size; i++) {
        grid->push(new VArray<Space*>(0, size));
        overlay->push(new VArray<char>(0, size));
        for(int j=0; j<size; j++) {
            grid->get(i)->push(new Space());
            overlay->get(i)->push(' ');
        }
    }
}
...
VArray<AbsolutePosition>* Board::getNeighboursForSpaceAt(AbsolutePosition pos) {
    VArray<AbsolutePosition> *neighbours = new VArray<AbsolutePosition>();
    RelativePosition checkRelPos = RelativePosition(up, 0);
    
    // check each adjacent space
    for(int _=0; _<4; _++) {
        std::cout << checkRelPos.x << ", " << checkRelPos.y << std::endl;
        AbsolutePosition candidatePos = checkRelPos.getAbsolutePos(pos);

        // valid neighbours have no entities on them and are within the board.
        if(checkPosWithinBoard(candidatePos) && getEntityAt(candidatePos) == NULL) {
            neighbours->push(candidatePos);
        }

        // rotate checkRelPos to the right
        RotationHandler::rotate90(checkRelPos, &pos, clockwise);
    }
